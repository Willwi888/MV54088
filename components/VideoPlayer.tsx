  ...
  const handleExport = useCallback(async () => {
    ...
    ffmpeg.on('progress', ({ progress }) => {
        ...
        setExportProgress(prev => ({
            ...
            details: `進度: ${(progress * 100).toFixed(0)}%`
        }));
    });
    ...
    try {
        ...
        if (!audioResponse.ok) throw new Error(`無法讀取音訊檔案: ${audioResponse.statusText}。這可能是 CORS 跨域問題，建議您下載後改用「上傳檔案」功能。`);
        await ffmpeg.writeFile('audio.mp3', new Uint8Array(await audioResponse.arrayBuffer()));
      
        setExportProgress({ message: '正在讀取素材...', progress: 7, details: '背景圖片' });
        const imageResponse = await fetch(imageUrl);
        if (!imageResponse.ok) throw new Error(`無法讀取圖片檔案: ${imageResponse.statusText}。這可能是 CORS 跨域問題，建議您下載後改用「上傳檔案」功能。`);
        await ffmpeg.writeFile('background.jpg', new Uint8Array(await imageResponse.arrayBuffer()));
        ...
        if (totalDuration === 0) throw new Error("音訊長度為 0，無法匯出影片。");
        const frameRate = 30;
        ...
        await new Promise((resolve, reject) => {
            ...
            bgImage.onerror = () => reject(new Error("背景圖片載入失敗。這可能是 CORS 跨域問題，請改用上傳功能。"));
        });
      
        const albumImage = includeAlbumArt ? bgImage : null;
      
        setExportProgress({ message: '正在準備字體...', progress: 10, details: `載入 ${fontFamily}` });
        try {
            await document.fonts.load(`${fontWeight} ${fontSize}px "${fontFamily.replace(/"/g, "")}"`);
        } catch (fontError) {
            console.warn("字體無法在渲染前載入，將使用瀏覽器預設字體:", fontError);
        }
        ...
        const drawFrame = (time: number) => {
            ...
            for(let i = -2; i <= 2; i++) {
                ...
                if (i === 0) {
                    ...
                    gradient.addColorStop(0, theme.active);
                    gradient.addColorStop(clampedProgress, theme.active);
                    gradient.addColorStop(clampedProgress + 0.001, theme.inactive1);
        ...
        for (let i = 0; i < totalFrames; i++) {
            ...
            await ffmpeg.writeFile(`frame${i.toString().padStart(5, '0')}.jpg`, new Uint8Array(arrayBuffer));
        ...
        const videoBlobUrl = URL.createObjectURL(new Blob([(data as Uint8Array).buffer], { type: 'video/mp4' }));

        const link = document.createElement('a');
        link.href = videoBlobUrl;
        const safeSongTitle = songTitle.replace(/[\/\\?%*:|"<>]/g, '_');
        const safeArtistName = artistName.replace(/[\/\\?%*:|"<>]/g, '_');
        link.download = `${safeSongTitle} - ${safeArtistName} (Lyrics).mp4`;
        ...
        if ((error as Error)?.message !== "Export cancelled") {
            alert(`匯出時發生錯誤: ${(error as Error).message}`);
        }
        setExportProgress(null);
    } finally {
        if (ffmpeg.loaded) {
        ...
        <div className="w-full lg:w-[400px] h-full flex-shrink-0 bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700 p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-6">
            ...
            <details open className="space-y-4">
              ...
              {showPerfWarning && (
                <div className="p-3 bg-yellow-900/50 border border-yellow-700 rounded-lg text-xs text-yellow-200">
                    <p><strong>效能提醒：</strong> 您的瀏覽器環境不支援最高效能運作，影片匯出可能會非常緩慢。為獲得最佳體驗，請網站管理員啟用 COOP/COEP 標頭。</p>
                </div>
              )}
              <button
